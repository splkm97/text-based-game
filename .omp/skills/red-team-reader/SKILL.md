...
3. 각 반응에 `Location`(장면 제목 또는 장면 id), `Evidence`(원문 인용 1~2문장), `Why Reader May React This Way`, `Confidence`(`High|Medium|Low`)를 붙인다. 완료 조건: 모든 반응에 다섯 필드가 채워져 있고 인용이 원문과 문자 단위로 일치한다.
