...
장면 대상, `RUN_ID`, 진입 상태를 얻을 정본 근거 중 하나라도 없으면 `input-blocked`로 처리하고 파일을 작성하지 않는다. 예외: 결말 장면처럼 선행 구간이 아직 설계되지 않아 진입 상태의 근거가 없는 역순 설계에서는 진입 상태를 `[HYPOTHESIS]`로 표시하고 진행할 수 있다 — 장면 대상과 `RUN_ID`는 이 경우에도 필요하다.
