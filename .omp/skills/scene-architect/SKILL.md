...
장면 대상, `RUN_ID`, 진입 상태를 얻을 정본 근거 중 하나라도 없으면 `input-blocked`로 처리하고 파일을 작성하지 않는다. 다만 선행 장면이 아직 설계되지 않은 역순 설계(결말 장면 먼저)에서는 진입 상태를 `[HYPOTHESIS]`로 표시하고 진행한다 — 장면 대상과 `RUN_ID`는 이 경우에도 필요하다. 이때도 `entry_state`·`exit_state` 비교로 `state change`를 `PASS|FAIL`로 판정하고, `UNVERIFIABLE`은 진입 상태 표기 자체가 빠진 경우로 한정한다.
