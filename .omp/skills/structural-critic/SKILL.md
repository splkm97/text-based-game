...
1. 진단 범위를 확정한다. 장면 목록과 각 장면의 구간(`파일:줄`)을 적고, 읽은 정본을 `RUN_DIR/inputs/manifest.json`과 스냅샷으로 기록한다. 완료 조건: 모든 장면에 `파일:줄`이 붙고 스냅샷 해시가 manifest와 일치한다.
...
3. 검사 항목 12개를 다음 순서로 적용해 후보 문제를 수집한다: 인과관계, 장면 필요성, 반복, 갈등 강도, 선택의 비용, 우연 의존, deus ex machina, 중반부 정체, stakes 변화, climax 준비, character agency, setup/payoff.
   - 후보마다 핵심 질문 7개를 모두 적용한다: 이 장면을 삭제하면 이후 이야기가 달라지는가? 이 장면에서 무엇이 변하는가? 문제가 인물의 선택으로 악화되는가? 갈등이 외부 사건만으로 발생하는가? 해결책이 이전에 준비되었는가? 주인공이 아닌 존재가 문제를 해결하지 않는가? 같은 갈등이 형태만 바뀌어 반복되는가? 완료 조건: 항목별 후보 목록이 있고 각 후보에 `파일:줄`이 있다.
...
- `[ISSUE]`마다 `위치`, `유형`, `근거`, `왜 문제인가`, `심각도`, `가능한 수정 방향 A`, `가능한 수정 방향 B`, `비고`를 포함한다. `위치`는 `파일:줄`, `유형`은 검사 항목 이름이다. `비고`는 후보에서 제외한 이유나 추가 확인 필요 항목만 적고, 없으면 `없음`으로 쓴다.
- 장면별 절: 대상 장면마다 한 행에 `Scene ID`, `파일:줄`, 삭제 테스트 결과, 변화 테스트 결과, 필요성 등급(`Essential|Contributing|Optional|Redundant`)을 적는다.
