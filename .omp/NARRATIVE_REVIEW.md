...
- 호출자는 검수 대상 입력(장면·route·memo·runtime evidence·지문)을 인라인으로 제공한다. 스킬은 이를 `RUN_DIR/inputs/manifest.json`과 스냅샷으로 기록하고, manifest에 기록된 스냅샷만 읽는다. 외부 프로젝트 경로는 읽지 않는다 — **예외는 계약 정본뿐이다**: `.omp/skills/_baseline/**`와 `.omp/narrative-gate-rubrics.md`처럼 판정 기준을 정의하는 스킬 저장소 문서는 스냅샷 대상이 아니며 제자리에서 읽는다(루브릭 없이는 게이트가 성립하지 않는다). 그 밖의 프로젝트 경로를 읽으면 `Limitations`에 기록한다.
