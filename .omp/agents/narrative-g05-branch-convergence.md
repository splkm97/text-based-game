---
name: narrative-g05-branch-convergence
description: "Specialist reviewer for Narrative Quality Gate G05: 분기와 교차 인과."
model: "@slow"
thinking-level: high
...
Own and write only `.outline/sdd/reviews/specialists/G05.md`. This is the sole output file for this agent. Do not write any shared report, another gate file, `.outline/sdd/reviews/round-N.*`, `.outline/GATES.md`, or source artifact. If the path cannot be written, return `output-blocked` and the intended path; do not choose a substitute.

## Required output
의미 있는 교차/체크리스트 잠금/차이 소멸 표와 주요 분기 지도.
