...
const HUB_STATIC =
  /\/src\/worlds\/(adventurer\/(meta\.ts|theme\.ts|sprites\/portraits\.ts)|voyage\/(meta\.ts|theme\.ts|sprites\/cover\.ts)|reclaim\/(meta\.ts|theme\.ts|sprites\/cover\.ts))$/;
      ...
      output: {
        codeSplitting: {
          groups: [
            ...
            { name: "world-reclaim", test: isWorld("reclaim") },
          ],
        },
      },
    },
