...
// content의 정본이라 여기서 자르거나 재작성하지 않고 그대로 출력한다: heading은
...
// 그 자리는 화면이 남긴 **여유**다: 이 열의 마지막 행(`ActionList`)에는 `mt-auto`가 없어서,
// 여유를 아무도 흡수하지 않으면 행동 버튼이 화면 중간에 떠 있고 아래가 빈 채로 남는다
// (실측: 1600px 뷰포트에서 버튼 아래 217px). 내용이 화면보다 길면 여유가 없으므로 이 클래스는
// 아무 일도 하지 않는다 — 좁은 화면에서 종이가 밀리지 않는 것과 같은 이유다.
